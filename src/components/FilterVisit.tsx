...
const FilterVisit: React.FC = () => {
    ...
    function MakeQuery (
        expectedStartTime : moment.MomentInput, expectedEndTime : moment.MomentInput, visitQuantity : number[]
        ,  scheduleTypeId : [], visitStatus : string[] | undefined
    ){
        var status = "["
  ...
  return (
    <div className={`fixed top-0 right-0 bg-white shadow-2xl rounded-l-md w-96 h-full grid grid-rows-[60px_1fr_60px] transform transition-transform duration-500
        ${
            statusTabs === false? "translate-x-full":""
        }
    `}>
        <div dir="ltr" className='grid grid-cols-5'>
        <h2 className='ps-5 col-span-4 text-black text-2xl pt-5 font-bold'><FilterOutlined />Bộ lọc tìm kiếm</h2>
        <button className=' text-black' onClick={handleFilterTabs}>X</button>
        </div>
        <div className='grid-rows-4 overflow-y-auto block'>
            <div className='p-5 grid border-b-2 border-b-gray-400 h-fit'>
                ...
                <DatePicker size='small' value={dayjs(expectedStartTime.format())} onChange={handleStartDateChange}/>
                </div>
                <div className='ps-5 pt-3 inline-flex grid-cols-5'>
                <p className='col-span-4 pe-5'>Ngày kết thúc</p>
                <DatePicker size='small' value={dayjs(expectedEndTime.format())} minDate={dayjs(expectedStartTime.format())} onChange={handleEndDateChange}/>
                </div>
            </div>
            <div className='p-5 grid border-b-2 border-b-gray-400 h-fit'>
            <h3 className='text-black text-xl pt-5 font-semibold'>Số Lượng Người</h3>
            <Slider range min={1} max={100} value={visitQuantity} onChange={SetVisitQuantity}/>
            </div>
            <div className='p-5 grid border-b-2 border-b-gray-400 h-fit'>
        ...
        <div className='grid grid-cols-2'>
            <button className='border-2 border-black text-black rounded-l-md font-bold' onClick={handleCancelFilter}>Xóa Tất Cả</button>
            <button className='bg-[#34495e] text-white font-bold' onClick={handleSubmitFilter}>Áp dụng</button>
        </div>
